...
const ensureAdminTables = async () => {
  ...
  if (existingConfig.rows.length === 0) {
    await query(`INSERT INTO public.app_configs (current_version, force_update, active) VALUES ('33.0.0', false, true)`);
  }

  // S'assurer que les documents légaux existent avec la version 33.0.0
  const existingLegalDocs = await query('SELECT type, version FROM public.app_legal_docs');

  if (existingLegalDocs.rows.length === 0) {
    // Initialiser avec des documents légaux par défaut pour la version 33.0.0
    await query(`INSERT INTO public.app_legal_docs (type, content, version, force_acceptance) VALUES 
      ('tos', 'Conditions Générales d''Utilisation - Version 33.0.0', '33.0.0', false),
      ('privacy', 'Politique de Confidentialité - Version 33.0.0', '33.0.0', false)`);
  } else {
    // Si les documents existent mais sont en version ancienne, on les met à jour en version 33.0.0
    await query(`
      UPDATE public.app_legal_docs
      SET version = '33.0.0',
          content = CASE
            WHEN type = 'tos' THEN 'Conditions Générales d''Utilisation - Version 33.0.0'
            ELSE 'Politique de Confidentialité - Version 33.0.0'
          END,
          force_acceptance = false
      WHERE version IN ('5.0.0', '1.0.0', '31.0.0', '32.0.0')
    `);
  }
